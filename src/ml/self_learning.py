...
try:
    ...
    from src.core.monitoring.alerting import get_alerts
except ImportError:
...
if __name__ == "__main__":
    ...
    detector = ConceptDriftDetector(window_size=50, threshold=0.2)
    
    # Simulate predictions
    for i in range(60):
        actual = np.random.choice([0, 1])
        predicted = actual if i < 40 else 1 - actual  # Force drift
        detector.add_prediction(actual, predicted)
    
    drift = detector.check_drift()
    print(f"   ✅ Drift detected: {drift}")
    
    print("\n✅ All self-learning tests passed!")
